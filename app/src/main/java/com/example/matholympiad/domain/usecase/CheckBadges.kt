...
class CheckBadges(private val userRepo: UserRepo) {
    ...
    suspend operator fun invoke(): Result<List<String>> = withContext(Dispatchers.IO) {
        try {
            ...
            // 检查"新手入门"勋章
            val badgesList = user.getBadgesList()
            if (user.todayQuestionsCompleted > 0 && !badgesList.contains("beginner")) {
                newBadges.add("beginner")
            }
            
            // 检查"坚持之星"勋章（连续打卡 7 天）
            if (user.streakCount >= 7 && !badgesList.contains("persistence_7days")) {
                newBadges.add("persistence_7days")
            }
            
            // 检查"数学小天才"勋章（总积分 100）
            if (user.totalScore >= 100 && !badgesList.contains("math_genius")) {
